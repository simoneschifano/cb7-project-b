...
const Quiz = ({data}) => {
  ...
  const startQuiz = () => {
    ...
    return <div className={styles.Quiz__main}>
        <h1>ASTROquiz</h1>
        {completedQuiz ?  null : <h2>{state.username}</h2>}
        <div className={styles.Close__Button} onClick={returnHome}>ᐅ</div>

        {completedQuiz ? null : <div className={styles.Quiz__Info}>
            <h3>Timer: {timer}</h3>
            <h4>Correct answers: {correctCounter}</h4>
            <h6>Given answers: {answerCounter}</h6>
        </div>}
        <div className={styles.Quiz__Container}>
            {startedQuiz === false && completedQuiz === false ? <div className={styles.Question__Container}>
                <button onClick={startQuiz}>Inizia Quiz</button>
            </div> : null
            }
            {
                startedQuiz && randomQuest && <div className={styles.Question__Container} key={randomQuest.id} ref={doneAnswer}>
                ...
                <div className={styles.Answers__Container} >
                    ...
                    {randomQuest && randomQuest.answers.map((answer, index) => {
                        return <p 
                            key={index} 
                            onClick={answer.isCorrect === true ? 
            ...
            {completedQuiz && <div className={styles.Question__Container}>
                                <div className={styles.Completed__Quiz}>
                                    <h2>Username</h2>
                                    <h3>Correct answers: <strong>{correctCounter}</strong></h3>
